...
func (cfg *apiConfig) handlerGetUserPosts(w http.ResponseWriter, r *http.Request, user database.User) {
	...
	if err != nil {
		utils.RespondWithError(w, http.StatusBadRequest, fmt.Sprintf("Couldn't get posts for a user: %v", err))
		return
	}

	utils.RespondWithJson(w, http.StatusOK, dbPostsToPosts(posts))
}
